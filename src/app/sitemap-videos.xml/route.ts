...
export async function GET() {
  ...
  // بناء sitemap للفيديوهات - مع صفحات مخصصة لكل فيديو
  const videosSitemap = Array.from(videosByPage.entries())
    .map(([pageUrl, videos]) => {
      // إنشاء صفحات منفصلة لكل فيديو
      return videos
        .map((video) => {
          const videoPageUrl = `${pageUrl.replace(/\/$/, '')}/video/${video.id}`;
          const videoContentUrl = video.src.startsWith('http') ? video.src : `${baseUrl}${video.src}`;
          const thumbnailUrl = video.thumbnail 
            ? (video.thumbnail.startsWith('http') ? video.thumbnail : `${baseUrl}${video.thumbnail}`)
            : `${baseUrl}/favicon.svg`;
          
          // تنظيف الـ URL من المسافات
          const cleanVideoUrl = videoContentUrl.replace(/\s+/g, '');
          const cleanThumbnailUrl = thumbnailUrl.replace(/\s+/g, '');
          const cleanVideoPageUrl = videoPageUrl.replace(/\s+/g, '');
    ...
    <video:video>
      ...
      <video:player_loc>${cleanVideoPageUrl}</video:player_loc>
